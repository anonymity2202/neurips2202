...
def main():
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets2.__dict__['chineseChars'])
            get_dataset = getattr(datasets2, 'chineseChars')
            num_classes = datasets2._NUM_CLASSES['chineseChars']
            train_loader, val_loader = get_dataset(
            ...
            assert callable(datasets2.__dict__['chineseChars_Dt'])
            get_dataset = getattr(datasets2, 'chineseChars_Dt')
            num_classes = datasets2._NUM_CLASSES['chineseChars_Dt']
            ...
            for k in range(len(added_example_indices)):
                ...
                teaching_example_index = teaching_example_index + 1
            fl.close()
            assert callable(datasets2.__dict__['chineseChars_Lt'])
            get_dataset = getattr(datasets2, 'chineseChars_Lt')
            num_classes = datasets2._NUM_CLASSES['chineseChars_Lt']
    ...
    np.save('./chinese_chars/all_train_acc_iter_' + args.selection + '_data_augment_imagenet4.npy', all_train_acc_iter)
    np.save('./chinese_chars/all_test_acc_iter_' + args.selection + '_data_augment_imagenet4.npy', all_test_acc_iter)
    # training evalution
