...
def main():
    ...
    if args.selection == 'MMT_largemargin':
        ...
        initial_weights = np.zeros((5, 512))
        initial_weights[0, 0] = 1.0
        initial_weights[1, 127] = 1.0
        initial_weights[2, 255] = 1.0
        initial_weights[3, 383] = 1.0
        initial_weights[4, 511] = 1.0
        initial_weights = torch.from_numpy(initial_weights).float()
        initial_weights = initial_weights.cuda()
        with torch.no_grad():
             model_main.fc.weight = torch.nn.Parameter(initial_weights)
    else:
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets.__dict__['butterflies'])
            get_dataset = getattr(datasets, 'butterflies')
            num_classes = datasets._NUM_CLASSES['butterflies']
            train_loader, val_loader = get_dataset(
            ...
            for k in range(len(added_example_indices)):
                ...
                teaching_example_index = teaching_example_index + 1
            fl.close()
            assert callable(datasets.__dict__['butterflies_Lt'])
            get_dataset = getattr(datasets, 'butterflies_Lt')
            num_classes = datasets._NUM_CLASSES['butterflies_Lt']
    ...
    np.save('all_train_acc_iter_' + args.selection + '_8.npy', all_train_acc_iter)
    np.save('all_test_acc_iter_' + args.selection + '_8.npy', all_test_acc_iter)
    # training evalution
