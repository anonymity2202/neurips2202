...
def main():
    ...
    teaching_set = './datasets/chinese_chars/' + args.selection + '/ChineseChars_STRICT_gt_tr.txt'
    teaching_example_index = 0
    all_test_acc_iter = np.zeros(args.iters)
    all_train_acc_iter = np.zeros(args.iters)
    teaching_set_index = [113, 139, 159, 297, 389, 24, 214, 153, 331, 229, 107, 517, 54, 12, 197, 502, 93, 271, 220, 364]
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets2.__dict__['chineseChars_STRICT'])
            get_dataset = getattr(datasets2, 'chineseChars_STRICT')
            num_classes = datasets2._NUM_CLASSES['chineseChars_STRICT']
