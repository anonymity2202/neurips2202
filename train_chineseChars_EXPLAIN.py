...
parser.add_argument('--Lambda', default=10.0, type=int, metavar='N',
                    help='see professor writing')
parser.add_argument('--selection', default='EXPLAIN', help='see professor writing')
...
def main():
    ...
    teaching_set = './datasets/chinese_chars/' + args.selection + '/ChineseChars_EXPLAIN_gt_tr.txt'
    teaching_example_index = 0
    all_test_acc_iter = np.zeros(args.iters)
    all_train_acc_iter = np.zeros(args.iters)
    teaching_set_index = [57, 355, 297, 220, 502, 457, 54, 12, 71, 441, 214, 197, 53, 194, 392, 58, 317, 331, 17, 29]
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets2.__dict__['chineseChars_EXPLAIN'])
            get_dataset = getattr(datasets2, 'chineseChars_EXPLAIN')
            num_classes = datasets2._NUM_CLASSES['chineseChars_EXPLAIN']
