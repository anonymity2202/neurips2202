...
def main():
    ...
    if args.selection == 'MMT_largemargin':
        ...
        initial_weights = np.zeros((3, 512))
        initial_weights[0, 0] = 1.0
        initial_weights[1, 255] = 1.0
        initial_weights[2, 511] = 1.0
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            assert callable(datasets.__dict__['chineseChars'])
            get_dataset = getattr(datasets, 'chineseChars')
            num_classes = datasets._NUM_CLASSES['chineseChars']
            train_loader, val_loader = get_dataset(
            ...
            assert callable(datasets.__dict__['chineseChars_Dt'])
            get_dataset = getattr(datasets, 'chineseChars_Dt')
            num_classes = datasets._NUM_CLASSES['chineseChars_Dt']
            ...
            for k in range(len(added_example_indices)):
                ...
                teaching_example_index = teaching_example_index + 1
            fl.close()
            assert callable(datasets.__dict__['chineseChars_Lt'])
            get_dataset = getattr(datasets, 'chineseChars_Lt')
            num_classes = datasets._NUM_CLASSES['chineseChars_Lt']
